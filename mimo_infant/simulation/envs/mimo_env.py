...
The abstract base class is :class:`~mimo_infant.simulation.envs.mimo_env.MIMoEnv`. Default parameters for all the sensory modalities
...
from mimo_infant.touch.touch import TrimeshTouch, Touch
from mimo_infant.vision.vision import SimpleVision, Vision
from mimo_infant.vestibular.vestibular import SimpleVestibular, Vestibular
from mimo_infant.proprioception.proprio import SimpleProprioception, Proprioception
from mimo_infant.actuation.actuation import ActuationModel, SpringDamperModel
...
class MIMoEnv(MujocoEnv, utils.EzPickle):
    ...
    - :meth:`._proprio_setup`, :meth:`._touch_setup`, :meth:`._vision_setup`, :meth:`._vestibular_setup`, these
      ...
      implementation. Default implementations are :class:`~mimo_infant.proprioception.proprio.SimpleProprioception`,
      :class:`~mimo_infant.touch.touch.DiscreteTouch`, :class:`~mimo_infant.vision.vision.SimpleVision`,
      :class:`~mimo_infant.vestibular.vestibular.SimpleVestibular`.
    ...
    Args:
        ...
        custom_measurements (dict | None): A dictionary of custom measurements for MIMo.  Keys must match measurement
            names from the ``mimo_infant/growth/measurements/`` folder, and values are floats representing measurements in centimeters.
        ...
        actuation_model (Type[ActuationModel]): Class for the actuation model. Default is
            :class:`~mimo_infant.actuation.actuation.SpringDamperModel`. Note that this must be a class, not an instance.
    ...
    def __init__(self,
        ...
        if self.age is not None:
            model_path = adjust_mimo_to_age(self.age, model_path, custom_measurements)
    ...
    def _initialize_simulation(self,):
        ...
        fps = int(np.round(1 / (model.opt.timestep * self.frame_skip)))
    ...
    def vision_setup(self, vision_params):
        ...
        for eye in vision_params:
            ...
            if (vision_params[eye]["acuity"] is True) and (type(vision_params[eye]["acuity"])==bool):
                vision_params[eye]["acuity"] = self.age if self.age is not None else 18
    ...
    def _set_action(self, action):
        ...
        Calls the actuation models function :meth:`mimo_infant.actuation.actuation.ActuationModel.action`. What exactly happens
