...
from mimo_infant.simulation.envs.mimo_env import MIMoEnv, SCENE_DIRECTORY, DEFAULT_PROPRIOCEPTION_PARAMS
...
class MIMoCatchEnv(MIMoEnv):
    ...
    def __init__(self,
        ...
        self.target_geoms = mimo_utils.get_geoms_for_body(self.model, self.target_id)
        self.own_geoms = []
        for body_name in touch_params["scales"]:
            body_id = self.model.body(body_name).id
            self.own_geoms.extend(mimo_utils.get_geoms_for_body(self.model, body_id))
        ...
        self.target_joint_qpos = mimo_utils.get_joint_qpos_addr(self.model, self.target_joint_id)
        self.target_joint_qvel = mimo_utils.get_joint_qvel_addr(self.model, self.target_joint_id)
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        while failing leads to a -100 penalty. Additionally, there is a per-step reward based on his hand's distance
        to the target and an action penalty based on the cost function of the actuation model.
        ...
        if self._currently_in_contact():
            ...
            hand_pos = mimo_utils.get_body_position(self.data, self.hand_id)
            target_pos = mimo_utils.get_body_position(self.data, self.target_id)
            reward = np.clip(0.5-np.linalg.norm(target_pos-hand_pos), -0.5, 0.5)
    ...
    def _step_callback(self):
        ...
        target_pos = mimo_utils.get_body_position(self.data, self.target_id) + self.position_offset
        head_pos = mimo_utils.get_body_position(self.data, self.head_id)
        ...
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:head_swivel",
                                 np.arctan(head_target_dif[1] / head_target_dif[0]))
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:head_tilt",
                                 np.arctan(-head_target_dif[2] / head_target_dif[0]))
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:head_tilt_side", 0)

        mimo_utils.set_joint_qpos(self.model, self.data, "robot:left_eye_horizontal",
                                 np.arctan(-half_eyes_dist / eyes_target_dist))
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:left_eye_vertical", 0)
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:left_eye_torsional", 0)

        mimo_utils.set_joint_qpos(self.model, self.data, "robot:right_eye_horizontal",
                                 np.arctan(-half_eyes_dist / eyes_target_dist))
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:right_eye_vertical", 0)
        mimo_utils.set_joint_qpos(self.model, self.data, "robot:right_eye_torsional", 0)
