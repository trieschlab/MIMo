...
def rotate_vector(vector: np.ndarray, rot_matrix: np.ndarray):
    ...
    :func:`~mimo_infant.simulation.utils.rotate_vector_transpose` for the inverse rotation. The exception are the contact frames,
...
def get_body_id(mujoco_model, body_id=None, body_name=None):
    ...
    Works identical to :func:`~mimo_infant.simulation.utils.get_geom_id`
...
def lock_joint(mujoco_model, joint_name, joint_angle=None):
    ...
    Args:
        ...
        joint_angle (float): The locking angle in radians, as a delta from the model starting value. The angle that the
            joint will be locked to can be set separately using :func:`~mimo_infant.simulation.utils.set_joint_locking_angle`. By
...
def unlock_joint(mujoco_model, joint_name):
    ...
    See :func:`~mimo_infant.simulation.utils.lock_joint`.
