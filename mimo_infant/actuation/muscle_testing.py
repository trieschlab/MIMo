...
from mimo_infant.simulation.utils import EPS
...
def create_joint_plots(plot_dir, data, dt=None):
    ...
    :meth:`~mimo_infant.simulation.envs.mimo_muscle_env.MIMoMuscleEnv.collect_data_for_actuator`.
...
def average_left_right(env, array):
    ...
    Args:
        env (mimo_infant.simulation.envs.MIMoEnv): A MIMo environment.
...
def compliance_test():
    ...
    shoulder_joint_qpos = mimo_utils.get_joint_qpos_addr(env.model, shoulder_joint_id)
    shoulder_joint_qvel = mimo_utils.get_joint_qvel_addr(env.model, shoulder_joint_id)
