...
The main class is :class:`~mimo_infant.simulation.envs.dummy.MIMoDummyEnv` which implements all methods from the base class as dummy
functions that returned fixed values. This allows for testing the model without the full gym bureaucracy.
The second class :class:`~mimo_infant.simulation.envs.dummy.MIMoShowroomEnv` is identical to the first, but changes the default
...
:class:`~mimo_infant.simulation.envs.dummy.MIMoV2DummyEnv`.
"""
...
from mimo_infant.simulation.envs.mimo_env import MIMoEnv, SCENE_DIRECTORY, DEFAULT_VISION_PARAMS, DEFAULT_VESTIBULAR_PARAMS, \
    DEFAULT_PROPRIOCEPTION_PARAMS, DEFAULT_TOUCH_PARAMS, DEFAULT_TOUCH_PARAMS_V2
from mimo_infant.touch.touch import TrimeshTouch
...
class MIMoDummyEnv(MIMoEnv):
    """ Dummy implementation for :class:`~mimo_infant.simulation.envs.mimo_env.MIMoEnv`.
    ...
    def touch_setup(self, touch_params):
        ...
        if self.show_sensors:
            for body_id in self.touch.sensor_positions:
                ...
                mimo_utils.plot_points(self.touch.sensor_positions[body_id], limit=1., title=body_name)
...
class MIMoV2DummyEnv(MIMoDummyEnv):
    """ Same as :class:`~mimo_infant.simulation.envs.dummy.MIMoDummyEnv`, but using the full hand version of MIMo which has hands with
...
class MIMoMuscleDummyEnv(MIMoDummyEnv):
    """Same as :class:`~mimo_infant.simulation.envs.dummy.MIMoDummyEnv`, but using the muscle actuation model. Uses the full hand
