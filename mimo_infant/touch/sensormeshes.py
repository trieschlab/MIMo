...
The functions here function much like those in :mod:`mimo_infant.touch.sensorpoints`, but the points are structured as a
...
import mimo_infant.touch.sensorpoints
...
def mesh_sphere(resolution, radius):
    ...
    points = mimo_infant.touch.sensorpoints.spread_points_sphere(resolution=resolution, radius=radius)
...
def mesh_ellipsoid(resolution, radii):
    ...
    Returns:
        trimesh.Trimesh: The output mesh.
    """
    points = mimo_infant.touch.sensorpoints.spread_points_ellipsoid(resolution=resolution, radii=radii)
    mesh = trimesh.points.PointCloud(points)
    if points.shape[0] == 1:
        return mesh
    else:
