...
The interface is defined as an abstract class in :class:`~mimo_infant.touch.touch.Touch`.
A simple implementation with a cloud of sensor points is in :class:`~mimo_infant.touch.touch.DiscreteTouch`.
A second implementation using trimesh objects is in :class:`~mimo_infant.touch.touch.TrimeshTouch`.
...
import mimo_infant.simulation.utils as mimo_utils
from mimo_infant.simulation.utils import rotate_vector_transpose, rotate_vector, EPS
from mimo_infant.touch.sensorpoints import spread_points_box, spread_points_sphere, spread_points_cylinder, \
                                   spread_points_capsule
from mimo_infant.touch.cache import StatsCache

#: A key to identify the geom type ids used by MuJoCo.
from mimo_infant.touch.sensormeshes import mesh_box, mesh_sphere, mesh_capsule, mesh_cylinder, mesh_ellipsoid
...
class DiscreteTouch(Touch):
    ...
    The following attributes are provided in addition to those of :class:`~mimo_infant.touch.touch.Touch`.
    ...
    def add_body(self, body_id=None, body_name=None, scale=math.inf):
        ...
        """
        body_id = mimo_utils.get_body_id(env.model, body_id=body_id, body_name=body_name)

        n_sensors = 0

        for geom_id in mimo_utils.get_geoms_for_body(env.model, body_id):
    ...
    def add_geom(self, geom_id=None, geom_name=None, scale=math.inf):
        ...
        """
        geom_id = mimo_utils.get_geom_id(env.model, geom_id=geom_id, geom_name=geom_name)
    ...
    def get_contact_position_relative(self, contact_id, geom_id):
        ...
        Returns:
            np.ndarray: An array with the position of the contact.
        """
        body_pos = mimo_utils.world_pos_to_geom(self.data, self.get_contact_position_world(contact_id), geom_id)
    ...
    def plot_sensors_geom(self, geom_id=None, geom_name=None):
        ...
        fig, ax = mimo_utils.plot_points(points, limit=limit, title=title, show=False)
        return fig, ax
    ...
    def plot_force_geom(self, geom_id=None, geom_name=None):
        ...
        if force_vectors.shape[1] == 1:
            ...
            fig, ax = mimo_utils.plot_forces(sensor_points, force_vectors, limit=np.max(sensor_points) + 0.5, show=False)
        return fig, ax
    ...
    def _get_plot_info_body(self, body_id):
        ...
        forces = []
        for geom_id in mimo_utils.get_geoms_for_body(env.model, body_id):
            points_in_body = mimo_utils.geom_pos_to_body(self.data, self.sensor_positions[geom_id], geom_id, body_id)
            points.append(points_in_body)
            force_vectors = self.sensor_outputs[geom_id] / 100
            if force_vectors.shape[1] == 1:
                # TODO: Need proper sensor normals, can't do this until trimesh rework
                raise RuntimeWarning("Plotting of scalar forces not implemented!")
            body_forces = mimo_utils.geom_rot_to_body(self.data, force_vectors, geom_id=geom_id, body_id=body_id)
    ...
    def plot_force_body(self, body_id=None, body_name=None):
        ...
        fig, ax = mimo_utils.plot_forces(points, forces, limit=np.max(points) + 0.5, show=False)
        return fig, ax
    ...
    def get_contact_normal(self, contact_id, geom_id):
        ...
        normal_vector = rotate_vector(normal_vector, mimo_utils.get_geom_rotation(self.data, geom_id))
    ...
    def force_vector(self, contact_id, geom_id):
        ...
        relative_forces = rotate_vector_transpose(global_forces, mimo_utils.get_geom_rotation(self.data, geom_id))
    ...
    def get_touch_obs(self):
        ...
        The indices of the output dictionary :attr:`~mimo_infant.touch.touch.DiscreteTouch.sensor_outputs` and the sensor
...
class TrimeshTouch(Touch):
    ...
    calls to :meth:`~mimo_infant.simulation.envs.mimo_env.MIMoEnv.reset`.
    ...
    def __init__(self, env, touch_params):
        ...
        cache_size = max(1, int(n_sensors / 20 + len(self.sensor_scales)))
        self._neighbour_cache = StatsCache(LRUCache(maxsize=cache_size))
    ...
    def add_body(self, body_id=None, body_name=None, scale=math.inf):
        ...
        Args:
            ...
            scale (float): The distance between sensor points.
        """
        body_id = mimo_utils.get_body_id(self.env.model, body_id=body_id, body_name=body_name)
        meshes = []
        for geom_id in mimo_utils.get_geoms_for_body(self.env.model, body_id):
            ...
            mesh.vertices = mimo_utils.geom_pos_to_body(self.env.data, mesh.vertices.copy(), geom_id, body_id)
            meshes.append(mesh)
    ...
    def get_contact_position_relative(self, contact_id, body_id):
        ...
        Returns:
            np.ndarray: An array with the contact position.
        """
        contact_pos = mimo_utils.world_pos_to_body(self.env.data, self.get_contact_position_world(contact_id), body_id)
    ...
    def get_raw_force(self, contact_id, body_id):
        ...
        contact = self.env.data.contact[contact_id]
        if contact.geom1 in mimo_utils.get_geoms_for_body(self.env.model, body_id=body_id):
            forces *= -1  # Convention is that normal points away from geom1
        elif contact.geom2 in mimo_utils.get_geoms_for_body(self.env.model, body_id=body_id):
            pass
        else:
    ...
    def get_contact_normal(self, contact_id, body_id):
        ...
        normal_vector = mimo_utils.rotate_vector_transpose(normal_vector, mimo_utils.get_body_rotation(self.env.data, body_id))
        return normal_vector
    ...
    def force_vector_global(self, contact_id, body_id):
        ...
        force_rot = np.reshape(contact.frame, (3, 3))
        global_forces = mimo_utils.rotate_vector_transpose(forces, force_rot)
        return global_forces
    ...
    def force_vector(self, contact_id, body_id):
        ...
        relative_forces = mimo_utils.rotate_vector_transpose(global_forces, mimo_utils.get_body_rotation(self.env.data,
                                                                                                       body_id))
        return relative_forces
    ...
    def normal_force(self, contact_id, body_id):
        ...
        normal_force = mimo_utils.world_rot_to_body(self.env.data, normal_force, body_id)
        return normal_force
    ...
    def get_touch_obs(self):
        ...
        The indices of the output dictionary :attr:`~mimo_infant.touch.touch.TrimeshTouch.sensor_outputs` and the sensor
    ...
    def plot_sensors_body(self, body_id=None, body_name=None, title=""):
        ...
        mimo_utils.plot_points(points, limit=limit, title=title)
    ...
    def plot_force_body(self, body_id=None, body_name=None, title=""):
        ...
        fig, ax = mimo_utils.plot_forces(sensor_points, force_vectors, limit=np.amax(sensor_points)*1.2, title=title,
                                        show=False)
        return fig, ax
    ...
    def plot_force_bodies(self, body_ids=[], body_names=[],
        ...
        if len(body_ids) == 0:
            body_ids = [mimo_utils.get_body_id(self.env.model, body_name=body_name) for body_name in body_names]
        ...
        for body_id in body_ids:
            ...
            if focus == "world":
                sensor_points_common = mimo_utils.body_pos_to_world(self.env.data, position=sensor_points, body_id=body_id)
                forces_common = mimo_utils.body_rot_to_world(self.env.data, vector=force_vectors, body_id=body_id)
            else:       # Focus == first
                sensor_points_common = mimo_utils.body_pos_to_body(self.env.data, position=sensor_points,
                                                                  body_id_source=body_id, body_id_target=body_ids[0])
                forces_common = mimo_utils.body_rot_to_body(self.env.data, vector=force_vectors,
                                                           body_id_source=body_id, body_id_target=body_ids[0])
        ...
        fig, ax = mimo_utils.plot_forces(points=points, vectors=forces, limit=limit, title=title, show=False)
        ...
        if show_contact_points:
            for contact_id, contact_body_id, forces in self.contact_tuples:
                if contact_body_id in body_ids:
                    ...
                    if focus == "world":
                        contact_position = mimo_utils.body_pos_to_world(self.env.data,
                                                                       position=contact_position,
                                                                       body_id=contact_body_id)
                    else:
                        contact_position = mimo_utils.body_pos_to_body(self.env.data,
                                                                      position=contact_position,
    ...
    def plot_force_body_subtree(self, body_id=None, body_name=None, title="", show_contact_points=False):
        ...
        subtree = mimo_utils.get_child_bodies(self.env.model, body_id)
    ...
    def visualize_contacts_subtree(self, root_id=None, root_name=None, show_contact_points=False, show_body_parts=False,
        ...
        def get_bodypart_vis(body_id, offsets=0):
            ...
            vertex_position = mimo_utils.body_pos_to_world(self.env.data, position=body_mesh.vertices,
                                                          body_id=body_id) + offsets
            triangles = body_mesh.faces
            return vertex_position, triangles

        root_id = mimo_utils.get_body_id(self.env.model, body_id=root_id, body_name=root_name)
        points_no_contact = []
        points_contact = []
        contact_magnitudes = []
        # Go through all bodies and note their child bodies
        ...
        for body_id in subtree:
            if (body_id in self.sensor_positions) and (body_id in self.sensor_outputs):
                ...
                no_touch_points = mimo_utils.body_pos_to_world(self.env.data, position=no_touch_points, body_id=body_id)
                touch_points = mimo_utils.body_pos_to_world(self.env.data, position=touch_points, body_id=body_id)
...
_vectors = mimo_utils.normalize_vectors(_rng.normal(size=(10, 3)))
